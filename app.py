...
# Business logic — all pulled from env, flexible from Render dashboard
FEE_PERCENTAGE             = float(os.getenv('FEE_PERCENTAGE', '1'))
MIN_TX_AMOUNT_ETH          = float(os.getenv('MIN_TX_AMOUNT_ETH', '0.1'))
GAS_PRICE_CAP_GWEI         = int(os.getenv('GAS_PRICE_CAP_GWEI', '50'))
VERIFICATION_CONFIRMATIONS = int(os.getenv('VERIFICATION_CONFIRMATIONS', '3'))
POLL_INTERVAL              = int(os.getenv('POLL_INTERVAL', '15'))

# Derived constants (auto-calculated from env vars, never hardcoded)
MIN_TX_AMOUNT_WEI = int(MIN_TX_AMOUNT_ETH * 1e18)
GAS_UNIT_LIMIT    = 21_000
GAS_BUFFER_WEI    = GAS_PRICE_CAP_GWEI * GAS_UNIT_LIMIT * 10**9  # user always pays this
...
print(f"   Gas cap        : {GAS_PRICE_CAP_GWEI} Gwei (buffer={w3.from_wei(GAS_BUFFER_WEI,'ether')} ETH)")
...
# Prevents nonce collisions when queue sends multiple TXs
# =============================================================
_nonce_lock    = threading.Lock()
_nonce_cache   = None

def get_next_nonce():
    """Return next nonce atomically. Resets cache on chain reorg or error."""
    global _nonce_cache
    with _nonce_lock:
...
def queue_push(job: dict):
    """Push a forward or refund job onto the queue."""
...
def _queue_sender_loop():
    """
    Single background thread that drains the forward queue one job at a time.
    Sequential processing guarantees nonce order.
    """
...
def _process_queue_job(job):
    ...
    # Read ACTUAL gas price from chain at send time
    try:
    ...
    if job_type == 'forward':
        ...
        # Option B: Priority order for covering actual gas
        # 1st: gas buffer (user pre-paid)
        # 2nd: platform fee (if buffer not enough)
        # 3rd: recipient amount (extreme edge case)
        ...
        print(f"\U0001f4e4 FORWARDED: {tx_hash[:10]}... \u2192 {forward_hash[:10]}... | actual_gas={w3.from_wei(actual_gas_cost_wei,'ether'):.6f} ETH | surplus={w3.from_wei(gas_surplus_wei,'ether'):.6f} ETH")
...
# Start queue sender thread at import time
...
def step_pending(tx_hash, tx_data, current_block):
    """PENDING -> VERIFIED: confirm escrow deposit on-chain (3 confirmations)."""
    try:
...
def step_verified(tx_hash, tx_data):
    """
    VERIFIED -> push to ForwardQueue immediately.
    Gas is read at actual send time inside the queue worker (Option B).
    No waiting, no retries, always forwards.
    """
    try:
        recipient_wei    = int(tx_data['recipient_amount_wei'])
        buffer_wei       = int(tx_data.get('gas_buffer_wei', str(GAS_BUFFER_WEI)))
        # Fee calculated HERE before any send (fixes bug: fee was calculated after)
...
def step_forwarding(tx_hash, tx_data, current_block):
    """FORWARDING_PENDING -> COMPLETE: wait for 3 confirmations on forward tx."""
    try:
        ...
        if not receipt:
            return
        if receipt['status'] != 1:
            # Forward reverted on-chain — refund user
            db_update(tx_hash, {
...
def step_refund(tx_hash, tx_data):
    ...
    try:
        ...
        queue_push({
            ...
            'tx_hash':     tx_hash,
            'destination': sender,
            'send_wei':    total_paid_wei,
        })
        db_update(tx_hash, {'status': 'refunding_queued', 'refund_queued_at': _now_iso()})
    except Exception as e:
...
def auto_cleanup(tx_hash, tx_data):
    """Delete terminal transactions 24h after completion (safe polling window)."""
    try:
...
def monitor_transactions():
    ...
    print(f"\n\U0001f916 Monitor started | poll={POLL_INTERVAL}s")
...
def get_config():
    """Public endpoint — frontend fetches this on load to get live config."""
    return jsonify({
...
def create_transaction():
    ...
    try:
        ...
        if int(recipient_amount_wei) < MIN_TX_AMOUNT_WEI:
            return jsonify({
                'error': f'Amount below minimum. Min: {MIN_TX_AMOUNT_ETH} ETH'
            }), 400
